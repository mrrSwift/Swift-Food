...
export const requestPayment = async (c: Context) => {
  ...
  if (method === "zarinpal") {
    ...
    if (result.data?.code === 100) {
      ...
      return c.json({
        success: true,
        data: {
          redirectUrl: `https://sandbox.zarinpal.com/pg/StartPay/${result.data.authority}`,
        },
      });
    } else {
      throw new AppError(
        result.errors?.message || "Zarinpal request failed",
        400,
      );
    }
  }
  ...
  if (method === "stripe") {
    ...
    order.payment = {
      method: "stripe",
      status: "pending",
      authority: session.id, // reuse authority field for session ID
    };
    await order.save();
...
export const verifyPayment = async (c: Context) => {
  ...
  if (order.payment?.method === "zarinpal") {
    ...
    if (Status !== "OK") {
      ...
      return c.json({
        success: true,
        data: { success: false, message: "Payment cancelled by user" },
      });
    }
